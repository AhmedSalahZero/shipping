                            ...
                            <div class="tab-pane active" id="tab_0">
                                <div class="portlet box green">
                                    <div class="portlet-title">
                                        <div class="caption">
                                            <i class="fa fa-gift"></i>@lang('lang.Assign To Courier')</div>
                                    </div>
                                    <div class="portlet-body form assign_to_courier_data">
                                        ...
                                        <div class="form_data_assign" style="display: none" >
                                            ...
                                            <div class="confirm_assign_order_div">
                                                <button class="btn blue confirm_assign_order_btn"> @lang('lang.Assign') </button>
...
//clear datatable
        ...
        $(document).on('change','#receive_barcode_input_assign',function(){
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "/admin/CheckItemTrackingNumberForAssign/"+trackingNo,
                data: {
                    '_token':"{{csrf_token()}}",
                },
                success: function (data) {
                    if(data.status)
                    {
                        ...
                        if(data.item.previous_status == 'reschedule')
                        {


                            if(! $('.body_for_products_assign tr').hasClass(`delete_assign_row_${data.item.barcode_number}`))
                            {

                                $('.body_for_products_assign').append(
                                    `<tr style="text-align: center" class="row_for_receive_order delete_assign_row_${data.item.barcode_number}" data-barcodeNo="${data.item.barcode_number}">
<td class="td_for_receive_order" > ${++order} </td>
<td> ${data.item.barcode_number} </td>
<td>

${  data.item.transPrevStatus + ' ( ' + data.item.scheduling_times + ' )' }

</td>

<td> ${data.item.transStatus} </td>
<td> ${data.item.client_name} </td>
<td> ${data.item.address}  </td>
<td> ${data.item.phone}  </td>
<td> ${data.item.price} @lang("lang.egp") </td>
<td> ${data.item.area_name}  </td>

<td> ${data.item.seller_name}  </td>
<td >
<i style="cursor: pointer;color: red" class="fa fa-remove fa-lg delete_assign" data-barcode_number="${data.item.barcode_number}">
</td>
</tr>`
                                );
                            }
                            else{
                                alert('This Shipping Already Exist');
...
<td> ${data.item.seller_name}  </td>
<td >
<i style="cursor: pointer;color: red" class="fa fa-remove red fa-lg delete_assign" data-barcode_number="${data.item.barcode_number}">
</td>
</tr>`
                                );
                            }
                            else{
                                alert('@lang('lang.This Shipping Already Exist')');
        ...
        $(document).on('click','.confirm_assign_order_btn',function(){
            ...
            $.ajax({
                url:"/admin/assignToUser/"+elements+'/'+courier_id ,
                type:"post",
                data:{
                    '_token':"{{csrf_token()}}",
                },
